<?php
/*
Plugin Name: Call to Action Post Thumbnail
Plugin URI: http://mertonium.com
Description: This plugin allows WP writers to have a way to associate an image and a link with a specific post.  (Based off of Chris Scott's "Multiple Post Thumbnails")
Version: 0.1
Author: Mertonium
...
if (!class_exists('CtaPlugin')) {
	...
	class CtaPlugin {
		...
		public function add_attachment_field($form_fields, $post) {
			...
			$link = sprintf('<a id="%4$s-%1$s-thumbnail-%2$s" class="%1$s-thumbnail" href="#" onclick="CtaPluginSetAsThumbnail(\'%2$s\', \'%1$s\', \'%4$s\', \'%5$s\');return false;">Set as %3$s</a>', $this->id, $post->ID, $this->label, $this->post_type, $ajax_nonce);
		...
		public function enqueue_admin_scripts() {
			wp_enqueue_script("featured-image-custom", plugins_url(basename(dirname(__FILE__)) . '/js/cta-post-thumbnail-admin.js'), array('jquery'));
		}
		...
		private function post_thumbnail_html($thumbnail_id = NULL) {
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
					...
					$content .= sprintf('<p class="hide-if-no-js"><a href="#" id="remove-%1$s-%2$s-thumbnail" onclick="CtaPluginRemoveThumbnail(\'%2$s\', \'%1$s\', \'%4$s\');return false;">%3$s</a></p>', $this->post_type, $this->id, esc_html__( "Remove {$this->label}" ), $ajax_nonce);
				}
